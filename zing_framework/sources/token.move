...
module zing_framework::token {
    ...
    use sui::{balance::{Self, Supply, Balance}, event::emit, package, vec_map::{Self, VecMap}};
    ...
    /// An TokenCap should be generated for each new Asset we wish to represent
    ...
    public struct TokenCap<phantom T> has key, store {
        id: UID,
    ...
    public fun mint<T>(
        ...
        assert!(
            !vec_map::is_empty(&metadata) && value == 1 || vec_map::is_empty(&metadata) && value > 0,
            EUniqueAsset,
        );
    ...
    public fun join<T>(self: &mut TokenizedAsset<T>, other: TokenizedAsset<T>): ID {
    ...
    public fun burn<T>(cap: &mut TokenCap<T>, tokenized_asset: TokenizedAsset<T>) {
    ...
    public fun total_supply<T>(cap: &TokenCap<T>): u64 {
        cap.total_supply
    }

    /// Returns the value of the current circulating supply.
    public fun supply<T>(cap: &TokenCap<T>): u64 {
