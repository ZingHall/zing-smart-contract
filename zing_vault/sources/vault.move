...
module zing_vault::vault {
    ...
    public fun calc_rebalance_amounts<T, YT>(
        ...
        // their cap so we can easily calculate the remaining amounts to allocate
        let mut i = 0;
        let n = vector::length(&max_borrow_idxs_to_process);
        while (i < n) {
            let idx = *vector::borrow(&max_borrow_idxs_to_process, i);
    ...
    fun assert_ticket_values<T, TY>(
        ...
        while (i < n) {
            ...
            assert!(
                strategy_withdraw_info.to_withdraw == *vector::borrow(&to_withdraw_values, i),
                0,
            );
            i = i + 1;
        };
    ...
    fun test_withdraw_ticket_redeem() {
        ...
        let start_state_a = vec_map::get(&vault.strategies, &id_a);
        assert!(start_state_a.borrowed == 2500, 0);
        let start_state_a = vec_map::get(&vault.strategies, &id_b);
        assert!(start_state_a.borrowed == 1000, 0);
        let start_state_a = vec_map::get(&vault.strategies, &id_c);
        assert!(start_state_a.borrowed == 1000, 0);
    ...
    fun test_strategy_hand_over_profit() {
        ...
        assert!(
            tlb::final_unlock_ts_sec(&vault.time_locked_profit) == timestamp_sec(&clock) + 4666,
            0,
        );
